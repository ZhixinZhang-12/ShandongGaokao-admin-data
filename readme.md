...
# 前端表格查询
使用flask来调用其他python程序，由于我前端水平实在太差看不懂怎么改能在查询完成后再在flask页面中显示，只能先放到**另一个本地页面**，等后面会了在改。可以通过学校名称来查询学校的信息和从2020到2024年共5年的录取数据，并生成echarts表格呈现在另外一个本地网页中。在“表格查询”文件夹下运行app.py（记得改python路径）后可以在127.0.0.1:5000中看到输入框，输入并查询后可以在文件夹下面的[tableResult.html](表格查询\tableResult.html)本地网页中获得图表。或者直接在script.py中改参数运行也是更快的选择。。

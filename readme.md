...
# 概要
主要包括了自2020年新高考改革后山东省高考志愿相关信息,包括招生录取计划和各个批次志愿录取情况。数据使用python进行了简单的处理,最终主要以xlsx表格形式展现出来,并给与一个小的前端查询以更方便的获得信息。招生计划数据以本科为主,2024年包括专科数据。志愿录取数据以及一分一段表等均来自于招生考试院,录取计划数据则来自于志愿报考本科绿本&专科蓝本的电子版,**主要由于绿本蓝本数据不能保证完全准确,故仅供参考**  。  
...
| 表格查询                         | 用于较为方便的依照高校名称查询每一年招生信息和院校的信息,通过查询实时生成echarts图表                                               |
| 可视化图表                  | 用于较为方便的依照高校名称查询每一年招生信息和院校的信息,存储在本地echart网页图表中                                                |
...
## 山东省分数一分一段表
山东省教育招生考试院发布的一分一段表,由于采用赋分制,故一分一段表参考意义不会太大,基本上每年的数据分布基本相同。
...
# 前端表格查询
![正确查询样例](表格截图/flask前端查询.png)
运行appold.py即可在前端html中得到相应数据,需要有flask和pyecharts等库。
使用flask来调用其他python程序,由于我前端水平实在太差看不懂怎么改能在查询完成后再在flask页面中显示的好看点,只能先长这样了挺丑的,等后面会了在改。可以通过学校名称来查询学校的信息和从2020到2024年共5年的录取数据,并生成echarts表格呈现在输入框下面。在“表格查询”文件夹下运行app.py（记得改python路径）后可以在127.0.0.1:5000中看到输入框,输入并查询可以看见生成的7个表格。或者直接在script.py中改参数运行也是更快的选择。。
名称必须是准确的全称,虽然做了一定的容错设计但是也就是有而已
![错误查询样例](表格截图/flask错误查询.png)
